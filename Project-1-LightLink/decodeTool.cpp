...
namespace decode {
    ...
    // 判断像素是黑色还是白色（灰度阈值，用于处理JPEG压缩后的质量损失）
    inline bool isBlack(const cv::Vec3b& pixel) {
        // 使用灰度值判断：< 100 视为黑色，与二值化阈值保持一致
        return (pixel[0] + pixel[1] + pixel[2]) / 3 < 100;
    }

    inline bool isWhite(const cv::Vec3b& pixel) {
        // 使用灰度值判断：>= 100 视为白色，与二值化阈值保持一致
        return (pixel[0] + pixel[1] + pixel[2]) / 3 >= 100;
    }
    ...
    uint16_t readFrameNumber(const cv::Mat& frame) {
        ...
        for (int i = 0; i < 16; i++) {
            ...
            int y = fixed_col;
            if (x >= 0 && x < frame.rows && y >= 0 && y < frame.cols) {
                cv::Vec3b pixel = frame.at<cv::Vec3b>(x, y);
                bool bit = isBlack(pixel);
                number |= (bit ? 1 : 0) << i;
            }
            // 超出边界，默认为 0
    ...
    uint16_t calculateCheckCode(int data_len, const unsigned char* data, int frame_id, bool isStart, bool isEnd) {
        ...
        for (int i = 0; i < len; i += 2) {
            // 模拟编码时的有符号转换逻辑
            res ^= (int)(signed char)data[i] << 8 | (int)(signed char)data[i + 1];
        }
        res ^= (uint16_t)frame_id;
        // 计算与编码时一致的 style 值
        int style = 0;
